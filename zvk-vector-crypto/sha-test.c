...
static int
run_test(struct sha_test *test, struct sha_param *param)
{
	int len, padding;
	uint8_t *block;
	uint64_t *ptr;
...
{
	int rc, n;
	struct sha_test *test;

	n = sizeof(sha256_suites) / sizeof(*sha256_suites);

	for (int i = 0; i < n; i++) {
		test = sha256_suites[i].tests;
		printf("Running %s test suite... ", sha256_suites[i].name);

		for (int j = 0; j < sha256_suites[i].count; j++, test++) {
			rc = run_test(test, &sha256_param);
			if (rc != 0) {
